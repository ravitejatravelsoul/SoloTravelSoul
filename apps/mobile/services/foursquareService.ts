...
// Per-user daily search limit — applied when the feature is enabled.
// Prevents runaway costs if the app is left open or scripted.
...
// Dev-only structured log — shape stable so future monitoring can parse it.
...
export async function searchPlaces(
  ...
  if (options?.ll) searchParams.set('ll', options.ll);

  logApiCall('GET /v3/places/search', Object.fromEntries(searchParams));
  ...
  try {
    ...
    const items: DiscoverItem[] = data.results.map((r) => ({
      id: r.fsq_id,
      name: r.name,
